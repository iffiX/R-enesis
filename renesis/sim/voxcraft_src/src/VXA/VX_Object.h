...
public:
    ...
    // TODO: Memory clean up at destruction?
	//Amplitude
    inline void SetAmplitude(int Index, double Value) {pAmplitudes[Index] = Value;} // nac: sets the material index here
    inline double GetAmplitude(int Index) const {if (pAmplitudes) return pAmplitudes[Index]; return 1.0f;}
    inline void InitAmplitudeArray(int Size) {pAmplitudes = new double[Size]; std::fill(pAmplitudes, pAmplitudes + Size, 1.0f);}
    inline bool GetUsingAmplitude(void) {return usingAmplitude;}

    //Frequency
    inline void SetFrequency(int Index, double Value) {pFrequencies[Index] = Value;} // nac: sets the material index here
    inline double GetFrequency(int Index) const {if (pFrequencies) return pFrequencies[Index]; return 1.0f;}
    inline void InitFrequencyArray(int Size) {pFrequencies = new double[Size]; std::fill(pFrequencies, pFrequencies + Size, 1.0f);}
    inline bool GetUsingFrequency(void) {return usingFrequency;}
