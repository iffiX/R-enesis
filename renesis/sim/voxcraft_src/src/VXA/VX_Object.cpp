...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
	...
	// Read amplitude
    if (pXML->FindElement("Amplitude")){
        usingAmplitude = true;
        int voxCounter = 0;
        // std::cout << "found amplitude weights!" << std::endl;
        InitAmplitudeArray(X_Voxels*Y_Voxels*Z_Voxels);
        for (int i=0; i<Z_Voxels; i++)
        {
            std::string RawData;
            pXML->FindLoadElement("Layer", &RawData, true, true);
            ...
            {
                if (dataArray.size()<=k) {
                    printf("ERROR: Data in Amplitude.Layers is too short.\n");
                }
                if (pData[X_Voxels*Y_Voxels*i+k] > 0)
                {
                    SetAmplitude(voxCounter,atof(dataArray[k].c_str()));
                    voxCounter++;
                }
            }
        }
    ...
    {
        usingAmplitude = false;
    }

    // Read frequency
    if (pXML->FindElement("Frequency")){
        usingFrequency = true;
        int voxCounter = 0;
        // std::cout << "found frequency weights!" << std::endl;
        InitFrequencyArray(X_Voxels*Y_Voxels*Z_Voxels);
        for (int i=0; i<Z_Voxels; i++)
        {
            std::string RawData;
            pXML->FindLoadElement("Layer", &RawData, true, true);
            ...
            {
                if (dataArray.size()<=k) {
                    printf("ERROR: Data in Frequency.Layers is too short.\n");
                }
                if (pData[X_Voxels*Y_Voxels*i+k] > 0)
                {
                    SetFrequency(voxCounter,atof(dataArray[k].c_str()));
                    voxCounter++;
                }
            }
        }
	...
	if (pXML->FindElement("PhaseOffset")){ 
		...
		int voxCounter = 0;
		// std::cout << "found phase offset weights!" << std::endl;
