...
from renesis.env_model.patch import normalize, PatchModel, PatchSphereModel
    ...
    if len(changed_material_types) == 1:
        for plot_voxels, is_changed in (
            ...
            if is_changed:
                ...
                colors[plot_voxels != 0] = [0.7] * 3 + [1]
        ax.voxels(np.logical_or(unchanged_non_empty, changed), facecolors=colors)
    else:
        # no change, plot original voxels
        colors[unchanged_voxels != 0] = [0.7] * 3 + [1]
...
def plot_largest_connected_components(
    ...
    fig = plt.figure()
    ax = fig.add_subplot(projection="3d")
    ax.voxels(voxels != 0, facecolors=colors)
...
def draw_generation_process(trial_record: TrialRecord, show_epoch: int = -1):
    ...
    for i in range(len(best_robot_actions)):
        ...
        plt.savefig(
            f"generated_data/step={i + 1}.png", bbox_inches="tight", pad_inches=0.5
        )
        # plt.savefig(f"step={i+1}.pdf", bbox_inches="tight", pad_inches=0.5)
        # plt.show()
    fig = plot_largest_connected_components("Generated robot", env_model.voxels)
    end_frame = mplfig_to_npimage(fig)

    frames += [end_frame] * 5

    # plt.savefig(f"robot_{show_epoch}.pdf", bbox_inches="tight", pad_inches=0.5)
    plt.savefig(
        f"generated_data/robot_{show_epoch}.png", bbox_inches="tight", pad_inches=0.5
    )
    create_video(frames, path="generated_data/", filename=f"robot_{show_epoch}", fps=5)
